...
class CalculatorApp(ft.Container):
    ...
    def __init__(self):
        ...
        self.content = ft.Column(
            controls=[
                ...
                ft.Row(
                    controls=[
                        ScientificButton(text="log(x)", button_clicked=self.button_clicked),
    ...
    def button_clicked(self, e):
        ...
        if self.result.value == "Error" or data == "AC":
            ...
            self.result.value = self.calculate(
                float(self.result.value), 0, "x^2"
            )
            self.reset()
            
        elif data in ("10^x"):
            self.result.value = self.calculate(
                float(self.result.value), 0, "10^x"
            )
        
        elif data in ("log(x)"):
            self.result.value = self.calculate(
                float(self.result.value), 0, "log(x)"
            )
            
        elif data in ("1/x"):
            self.result.value = self.calculate(
                float(self.result.value), 0, "1/x"
            )
            
        elif data in ("x^y"):
            self.result.value = self.calculate(
                float(self.result.value), 0, "x^y"
            )
            
        elif data in ("!"):
            self.result.value = self.calculate(
                float(self.result.value), 0, "!"
            )
        self.update()
    ...
    def calculate(self, operand1, operand2, operator):
        ...
        if operator == "+":
            ...
            return self.format_number(math.log(operand1))
            ...
            if operand1 < 0:
                return "Error"
            return math.factorial(int(operand1))
